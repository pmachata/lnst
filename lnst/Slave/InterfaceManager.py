...
class InterfaceManager(object):
    ...
    def get_id_by_if_index(self, if_index):
        for if_id, index in list(self._id_mapping.items()):
            if if_index == index:
                return if_id
        return None
    ...
    def _handle_netlink_msg(self, msg):
        if msg['header']['type'] in [RTM_NEWLINK, RTM_NEWADDR, RTM_DELADDR]:
            if msg['index'] in self._devices:
                ...
                if update_msg != None:
                    for if_id, if_index in list(self._id_mapping.items()):
    ...
    def _is_name_used(self, name):
        ...
        for device in list(self._devices.values()):
            if name == device.get_name():
                return True
        for device in list(self._tmp_mapping.values()):
            if name == device.get_name():
                return True
        return False
...
class Device(object):
    ...
    def link_stats(self):
        ...
        for line in lines:
            ...
            if (line.split()[0] == 'vf'):
                break
            if (line.split()[0] == "RX:"):
                rx_stats = list(map(int, lines.__next__().split()))
                stats.update({"rx_bytes"  : rx_stats[0],
                              ...
                              "rx_mcast"  : rx_stats[5]})
            if (line.split()[0] == "TX:"):
                tx_stats = list(map(int, lines.__next__().split()))
    ...
    def qdisc_red_stats(self, parent = None):
        ...
        stats = {key: int(val.replace("K", "000").replace("M", "000000"))
                for key, val in list(stats_raw.groupdict().items())}
    ...
    class QdiscRedStatCollector:
        ...
        def stop(self):
            ...
            for key in list(self._old_stats.keys()):
                if key not in stats:
                    continue
                if type(stats[key]) in (int, int):
