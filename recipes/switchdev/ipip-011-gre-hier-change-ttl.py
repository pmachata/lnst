...
from ipip_common import ping_test, encap_route, \
                        ...
                        onet1_ip, onet2_ip, \
                        unet1_ip, unet2_ip, unet3_ip, ipv4, ipv6
...
def do_task(ctl, hosts, ifaces, aliases):
    ...
    m2_if1.add_nhs_route("1.2.3.4/32", [ipv4(unet1_ip(ctl, 1, []))])
    m2_if1.add_nhs_route("1.2.3.5/32", [ipv4(unet2_ip(ctl, 2, []))])
    m2_v3.add_nhs_route("1.2.3.4/32", [ipv4(unet2_ip(ctl, 1, []))])
    m2_v3.add_nhs_route("1.2.3.5/32", [ipv4(unet3_ip(ctl, 2, []))])
    m2_mg.add_nhs_route("1.2.3.4/32", [ipv4(unet3_ip(ctl, 1, []))])
    ...
    with vrf(sw) as vrf_u, \
        ...
        sw_if1.reset()
        sw_if2.reset()
        add_forward_route(sw, vrf_u, "1.2.3.5", via=ipv4(unet1_ip(ctl, 2, [])))
        ...
        with dummy(sw, vrf_u) as d, \
            ...
            sleep(5)
            ping_test(tl, m1, sw, ipv4(onet2_ip(ctl, 33, [])), m1_if1, g, ttl=4)
